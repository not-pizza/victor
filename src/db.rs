...
impl<D: DirectoryHandle> Victor<D> {
    ...
    pub(crate) async fn write(&mut self, embedding: Vec<f32>, content: &str) {
        let id = Uuid::new_v4();

        let embedding = Embedding { id, embedding };

        self.write_embedding(embedding).await;
        self.write_content(content, id).await;
    }

    pub(crate) async fn find_nearest_neighbor(&mut self, vector: Vec<f32>) -> Option<Content> {
    ...
    async fn write_embedding(&mut self, embedding: Embedding) {
        let mut victor_file_handle = self
            .root
            .get_file_handle_with_options("victor.bin", &GetFileHandleOptions { create: true })
            .await
            .unwrap();
    ...
    async fn write_content(&mut self, content: &str, id: Uuid) {
